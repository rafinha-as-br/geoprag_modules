...
/// Sub-Administrador) define o restante do fluxo: aqui simulado por
/// [mockAdminAccounts], já que o papel viria do backend em produção.
...
class _EsqueciSenhaWebScreenState extends State<EsqueciSenhaWebScreen> {
  ...
  void _prosseguir() {
    ...
    final conta = mockAdminAccounts
        .where((a) => a.email.toLowerCase() == email)
        .cast<AdminAccount?>()
        .firstWhere((a) => true, orElse: () => null);

    final role = conta?.role ?? AdminRole.subAdministrador;
    if (role == AdminRole.administrador) {
      Navigator.pushNamed(context, '/senha/codigo-admin');
    } else {
      Navigator.pushNamed(context, '/senha/aguardando');
    }
  }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Esqueci minha senha',
                    style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: GeopragColors.green900),
