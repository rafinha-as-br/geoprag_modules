...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<LoginCubit, AuthActionState<User>>(
      listener: (context, state) {
        if (state is AuthActionSuccess<User>) {
          AplicadorNavigatorScope.of(context).toPonto();
        } else if (state is AuthActionFailure<User>) {
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                BlocBuilder<LoginCubit, AuthActionState<User>>(
                  builder: (context, state) {
                    return ElevatedButton(
                      onPressed: state is AuthActionLoading<User>
                          ? null
                          : () => context.read<LoginCubit>().submit(
                              identifier: _identifierController.text,
                              senha: _senhaController.text,
                            ),
                      child: state is AuthActionLoading<User>
                          ? const SizedBox(
                              width: 20,
                              height: 20,
