...
void main() {
  ...
  test(
    'criar adiciona a conta como Sub-Administrador, mesmo cargo não sendo informado',
    () async {
  ...
  test(
    'criar lança EntidadeDuplicadaException para e-mail já cadastrado',
    () async {
      expect(
        () => repository.criar(
  ...
  group('desativar', () {
    test(
      'desativa o cadastro quando o executor é Administrador ativo',
      () async {
        final conta = await criarSubAdministrador(
          'sub-desativar@gaspar.sc.gov.br',
        );
    ...
    test(
      'lança OperacaoNaoPermitidaException quando o executor não é Administrador',
      () async {
    ...
    test(
      'cancela automaticamente votação de promoção em aberto para o alvo desativado',
      () async {
        adicionarAdministrador('admin2-desativa@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-cancelado@gaspar.sc.gov.br',
        );
        ...
        final solicitacao =
            (resultado as SolicitacaoPromocaoAberta).solicitacao;
    ...
    test('registra a data da desativação', () async {
      final conta = await criarSubAdministrador(
        'sub-data-desativar@gaspar.sc.gov.br',
      );
      ...
      expect(atualizado.dataDesativacao, isNotNull);
    });
  });

  group('reativar', () {
    test(
      'reativa o cadastro quando o executor é Administrador ativo',
      () async {
        final conta = await criarSubAdministrador(
          'sub-reativar@gaspar.sc.gov.br',
        );
        await repository.desativar(
          email: conta.email,
          executorEmail: _adminSeedEmail,
        ...
        expect(atualizado.ativo, isTrue);
      },
    );

    test(
      'mantém a data da última desativação registrada mesmo após reativar',
      () async {
        final conta = await criarSubAdministrador(
          'sub-mantem-data@gaspar.sc.gov.br',
        );
        await repository.desativar(
          email: conta.email,
          executorEmail: _adminSeedEmail,
        );
        final dataDesativacao = mockAdminAccounts
            .firstWhere((c) => c.email == conta.email)
            .dataDesativacao;
        ...
        expect(atualizado.dataDesativacao, dataDesativacao);
      },
    );

    test(
      'lança OperacaoNaoPermitidaException se o cadastro já está ativo',
      () async {
        final conta = await criarSubAdministrador(
          'sub-ja-ativo@gaspar.sc.gov.br',
        );
  ...
  group('rebaixar', () {
    test(
      'rebaixa o Administrador alvo quando o executor é Administrador ativo diferente dele',
      () async {
    ...
    test(
      'lança OperacaoNaoPermitidaException ao tentar rebaixar o próprio cargo',
      () async {
        expect(
          () => repository.rebaixar(
            email: _adminSeedEmail,
    ...
    test(
      'lança OperacaoNaoPermitidaException quando o executor não é Administrador ativo',
      () async {
    ...
    test(
      'lança OperacaoNaoPermitidaException ao tentar rebaixar quem já é Sub-Administrador',
      () async {
        final alvo = await criarSubAdministrador(
          'sub-nao-rebaixavel@gaspar.sc.gov.br',
        );
  ...
  group('solicitarPromocao', () {
    test(
      'promove automaticamente quando não há outro Administrador ativo elegível',
      () async {
        final alvo = await criarSubAdministrador(
          'auto-promovido@gaspar.sc.gov.br',
        );
    ...
    test(
      'abre votação travando a base de elegíveis quando há outros Administradores',
      () async {
        adicionarAdministrador('admin2-votacao@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-votacao@gaspar.sc.gov.br',
        );
    ...
    test(
      'lança OperacaoNaoPermitidaException se já existe votação aberta para o mesmo alvo',
      () async {
        adicionarAdministrador('admin2-duplicada@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-duplicado@gaspar.sc.gov.br',
        );
  ...
  group('votar', () {
    ...
    test(
      'encerra como reprovada ao atingir o limiar de votos contrários',
      () async {
        adicionarAdministrador('admin2-reprova@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-reprovado@gaspar.sc.gov.br',
        );
    ...
    test(
      'lança OperacaoNaoPermitidaException quando quem vota é o solicitante ou o alvo',
      () async {
        adicionarAdministrador('admin2-inelegivel@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-inelegivel@gaspar.sc.gov.br',
        );
  ...
  group('cancelarSolicitacao', () {
    ...
    test(
      'lança OperacaoNaoPermitidaException se quem cancela não é o solicitante',
      () async {
        adicionarAdministrador('admin2-nao-solicitante@gaspar.sc.gov.br');
        final alvo = await criarSubAdministrador(
          'sub-nao-solicitante@gaspar.sc.gov.br',
        );
