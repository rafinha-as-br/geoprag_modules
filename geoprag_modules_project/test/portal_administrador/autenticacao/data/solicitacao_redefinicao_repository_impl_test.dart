...
void main() {
  ...
  test('buscarPendente retorna a solicitação mockada com status "aguardando"', () async {
  ...
  test('autorizar muda o status para "autorizado" e persiste na instância', () async {
    final result = await repository.autorizar('sr1');

    expect(result.status, StatusSolicitacaoRedefinicao.autorizado);
    expect((await repository.buscarPendente()).status, StatusSolicitacaoRedefinicao.autorizado);
  });
  ...
  test('negar muda o status para "negado" e persiste na instância', () async {
    ...
    expect((await repository.buscarPendente()).status, StatusSolicitacaoRedefinicao.negado);
  });
