...
///
/// TODO(GEOPRAG-38): hoje `AdminNavigator.toAplicacaoDetalhes()` não carrega
/// um id — mesma limitação já documentada em
/// `gestao_de_aplicadores/presentation/aplicador_detalhe_cubit.dart` e
/// `mapa_hidrologico/presentation/bairro_detalhe_cubit.dart`.
class PontoDeAplicacaoDetalheCubit extends Cubit<PontoDeAplicacaoDetalheState> {
  ...
  Future<List<AplicadorOpcaoViewModel>> listarAplicadoresDisponiveis() async {
    final aplicadores = await _aplicadorRepository.listar();
    return aplicadores
        .where((aplicador) => aplicador.status == 'ativo')
